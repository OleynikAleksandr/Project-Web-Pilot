...
test('vendored Workflow Kit exactly matches WF001 20260a0 source snapshot', async () => {
  ...
  const files = (await fs.readdir(root, { recursive: true, withFileTypes: true })).filter(e => e.isFile() && !e.name.endsWith('.md')).map(e => path.relative(fileURLToPath(root), path.join(e.parentPath, e.name))).sort();
  assert.deepEqual(files, Object.keys(expected).filter(f => !f.endsWith('.md')).sort());
  for (const [file, sha] of Object.entries(expected).filter(([file]) => !file.endsWith('.md'))) assert.equal(createHash('sha256').update(await fs.readFile(new URL(file, root))).digest('hex'), sha, file);
});
