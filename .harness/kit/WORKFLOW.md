...
2. Единственный активный todo-plan управляется командами Workflow Kit; machine-managed поля вручную не менять.
3. Новый функциональный запрос сначала сопоставь с владельцем в `docs/MODULES.md`.
4. Для существующего модуля сначала согласуй изменение module specification; если владельца нет — сначала создай и согласуй новую specification.
5. Todo-plan реализует уже согласованный контракт модуля, а не проектирует архитектуру по ходу микрозадач.
6. Перед изменением начни task; каждая микрозадача завершается собственным проверенным commit.
7. Не обходи hooks и не удаляй/не откатывай посторонние изменения.
8. Cross-module работу по возможности разделяй по владельцам и небольшой интеграционной задаче.
9. Архивирование scope выполняется только по отдельной прямой команде пользователя.
...
При NONE обсуждай идею и требования. Для функционального изменения сначала найди владельца в `docs/MODULES.md`, согласуй module specification (или создай новую), затем ближайший scope. Выбор стека не обязателен для документального этапа. После согласования создай JSON-вход по `.harness/plans/todo-plan.template.md` и выполни `./scripts/workflow scope:create --input <файл>`. При BLOCKED разрешены диагностика и обсуждение причины; реализация ждёт решения.
...
Документы ищи через `docs/DOCUMENTATION_INDEX.md`. Меняй связанные контракты вместе с кодом; содержательную актуальность проверяет агент. Стек и команды проверок настрой через config:apply до функциональных задач. Ненастроенная проверка не считается пройденной. Сборка не заменяет запуск приложения и пользовательскую приёмку.

После всех задач сообщи готовность к приёмке. Scope остаётся ACTIVE. Только отдельная прямая команда пользователя разрешает archive; приёмка, завершение чата или цели такого разрешения не дают. Релиз/публикация требуют указания пользователя.
