...
В гостевой 0.6.37 подтверждена подготовка компонентов после Apple. В 0.6.38 исправлены следующий отказ нативного ввода туннеля и оставшаяся кнопка установки. Полный путь до доставки контекста и действия с гостевым файлом ещё не пройден. Контракт доставки не менялся; результаты компонентов не подтверждают MCP/tunnel или первый ответ агента.
