...
Текущая версия приложения — 0.6.15: «Ваши проекты» использует меню редких действий и компактное дерево. Нажатие на имя проекта открывает последнюю созданную сессию; новые сессии сверху, видны максимум три строки с отдельной заметной прокруткой. После принятия и штатного архивирования плана по-прежнему предлагается Chat/Work для новой сессии с актуальным контекстом. Workflow Kit — 1.2.0. Исторические проверки — в `docs/VERIFICATION.md`.
