...
Текущая версия приложения — 0.6.13: добавлены локальное переименование проектов/сессий и автоматическое имя текущей сессии по новому scope; быстрый контекст и спиннеры операций сохранены. Workflow Kit — 1.2.0. Детальные исторические проверки остаются в `docs/VERIFICATION.md` и не являются стартовым контекстом.
