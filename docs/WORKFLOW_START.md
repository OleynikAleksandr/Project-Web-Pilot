...
Текущая версия приложения — **0.6.24**, Workflow Kit — **1.3.0**. Сохранены дерево Chat/Work-сессий, переход в новую сессию после принятого и архивированного scope, автопрокрутка разговора, self-healing runtime и единый macOS/Windows source. В 0.6.22 скрытые строки вызовов инструментов больше не оставляют служебную пустоту в layout, поэтому низ автопрокрутки совпадает с последним видимым содержимым. В 0.6.19 добавлен project continuity: `NONE` больше не создаёт пустой контекст, каждый новый рабочий scope автоматически получает обязательную финальную актуализацию документации, а пользовательская приёмка остаётся отдельным gate. Исторические проверки и релизы — в `docs/VERIFICATION.md`.
...
## Релиз 0.6.24 — исправление цвета плашки

В Settings появилась кнопка «Цвета чата…»: отдельное перемещаемое окно, четыре цвета, немедленное применение, автосохранение и независимый/общий сброс. Поставки macOS arm64 и Windows x64 — `.harness/runtime/releases/0.6.24/`; пользовательские копии — `~/Downloads/WebPilot-0.6.24/`. Scope остаётся ACTIVE / READY_FOR_ACCEPTANCE до прямого принятия и отдельной команды на archive.
