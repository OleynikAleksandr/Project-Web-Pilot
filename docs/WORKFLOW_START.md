...
Текущая версия приложения — 0.6.9 с Recovery v2, self-healing Runtime Lifecycle, Chat/Work sessions и локальным счётчиком токенов прочитанной переписки. Workflow Kit — 1.2.0. Детальные исторические проверки остаются в `docs/VERIFICATION.md` и не являются стартовым контекстом.
