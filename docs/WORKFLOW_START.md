...
Текущая версия приложения — 0.6.11: подсчёт всей текстовой истории с загрузкой предыдущих страниц, явный статус неполной оценки, быстрый контекст и спиннеры операций. Workflow Kit — 1.2.0. Детальные исторические проверки остаются в `docs/VERIFICATION.md` и не являются стартовым контекстом.
