...
Текущая версия приложения — 0.6.10: предварительная подготовка полного контекста, спиннеры операций, Chat/Work sessions и локальный счётчик токенов. Workflow Kit — 1.2.0. Детальные исторические проверки остаются в `docs/VERIFICATION.md` и не являются стартовым контекстом.
