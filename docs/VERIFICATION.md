...
## Исторический снимок checkout до 0.6.28 — 17.09.2026
...
Unit и Electron проверяют оба первых режима, двойные клики, отмену, отсутствие лишней сессии, неверный тип до записи, healthy/error preview. Smoke проходит реальные стадии интерфейса с подставным clipboard и снимает кадры после отрисовки; реальные ключи не читаются.

### 0.6.39 — A005: Повтор после отказа туннеля

Интеграционная проверка связывает реальные readiness, clipboard и DOM: отказ, сохранение ID/ручного ввода, исправленный ключ и успех. Неудачная повторная проверка остаётся неготовностью.
...
Проверка существующего view покрывает пустой parent, единственное первое
действие, недоступность имени/отправки и переход к имени после выбора папки.

P002 / 0.6.40: Electron fixture проходит настоящий IPC создания проекта с подменой только ответа системного диалога на собственную временную папку. Проверяются отсутствие начального пути, отказ preview без выбора, отмена без скрытого значения, переход к имени с фокусом и создание первых Chat/Work после явного выбора. Снимки `new-project-location.png` и `new-project-name.png` показывают последовательность формы; native OS picker этим fixture не проверяется.

P003 / уточнение 0.6.40: Electron проверяет пустое начало при свежих настройках,
сохранение первого выбора в settings.json и повторное открытие формы сразу с
ним. Отмена смены сохраняет путь; новая папка записывается на диск и используется
следующим проектом Work. Другие изменения настроек не теряют projectsParent.
Снимки обычной формы снимаются после сброса намеренного отказа некорректного IPC.
Восстановление при запуске использует то же поле settings.json; физический
перезапуск пользовательского приложения этим fixture не выполняется.

Полный Electron smoke после уточнения прошёл. Тесты явно фокусируют fixture перед
проверкой записи буфера обмена; для sidebar fixture отключено фоновое замедление
анимаций, чтобы проверки кадров не зависели от перекрытия окна другим приложением.
Рабочие окна и их настройки этим изменением не затронуты.
...
Managed commit `978e6613e6ff4285083911a6d137cb12d37660cb`: syntax PASSED,
Node suite — 282 tests, 280 passed, 0 failed, 2 skipped (48.109 s);
Electron smoke PASSED (88.783 s). Evidence сохранён отдельно в
`.harness/runtime/releases/0.6.40/managed-verification.json`, поэтому финальный
документальный commit не заменяет запись проверки поставки. Электронный fixture
подтвердил первоначально пустую папку, сохранение выбора, повторное использование,
отмену смены, новый сохранённый путь и создание первой Work в нём. Пакеты и
постоянный app соответствуют исходникам этого commit. Пользовательская проверка
0.6.40 и нативная Windows отдельно не объявляются выполненными.

## Сверка существующего релиза перед закрытием 031

18.09.2026 повторно проверены фактические SHA-256 и размеры ZIP macOS arm64/Windows x64 в каталоге выпуска и Downloads, версия Info.plist и SHA-256 установленного app.asar. Совпадение подтверждено; функциональные файлы не менялись относительно проверенного release commit 978e6613e6ff4285083911a6d137cb12d37660cb. Локальный receipt — .harness/runtime/close-031-release-check.json.

Для этой поставки действуют записанные выше syntax, 282 Node tests (280 passed, 2 skipped) и Electron smoke. В документальном закрытии suite и smoke повторно не выдаются за новый запуск; управляемые коммиты выполняют обязательную syntax-проверку. Windows VM и native x64 этой сверкой не проверялись.


## DOCS — закрытие first-run-onboarding-031

Проверены README, AGENTS и 23 документа в docs по DOCUMENTATION_INDEX. Устранены устаревшие вводные версии, описание общего единственного плана, старые блокеры первого запуска и привязка общих инструкций к чужой сессии. Сохранены версионная история, известный отказ вывода окна Apple в 0.6.37, границы подтверждения 0.6.40 и отдельные статусы Windows. Локальные ссылки исходных внешних проектов представлены как пути, читаемые на GitHub.

Проверены 41 относительная Markdown-ссылка и наличие документов индекса; отсутствующих файлов не обнаружено. Все 23 файла установленного ядра совпадают с поставляемым Workflow Kit; ядро, шаблоны и manifest не изменялись. Контракты Project Doctor, Runtime Lifecycle, Workflow Kit recovery, архива проектов и принятый макет навигации просмотрены и оставлены без изменений. План first-run-corrections-032 побайтово сохранён. Локальные записи аудита — .harness/runtime/close-031-doc-{audit,check}.json.

Оставшиеся Windows-критерии переданы в docs/TRANSFER_TO_WINDOWS.md; закрытие 031 разрешено пользователем и не означает успешных Windows-испытаний. Каждая задача передачи имеет свой проверенный commit, DOCS — последняя. После неё выполняются штатный archive своей сессии и обычный push main на origin.

### Дополнительная проверка ссылок recovery — C013 / DOCS, итерация 2

Проверка обязательных секций всех канонических планов обнаружила ссылку плана 029 на заголовок «Действующая модель — 0.6.29 / schema v6». Заголовок сохранён, актуальное поведение 0.6.40 описано отдельным разделом. После исправления все 41 проверки обязательных документов/заголовков прошли. Чужие планы не изменялись; финальная DOCS повторно открыта штатным correction round. Ссылки и совпадение Workflow Kit перепроверены; приложение и состав 0.6.40 не менялись.

## Windows onboarding 0.6.41 — W001
...
0.6.41 / V001: release-all.test.mjs — 3 PASS. Проверены рассогласование версий,
отсутствующий Windows helper, устаревший source, изменение external resources,
вложенный старый app и остановка последовательной сборки при ошибке первой
платформы. Electron smoke расширен реальным Windows adapter + Chromium UI:
подготовка → туннель → открытая инструкция Plugins → первый проект; Apple
probe/install запрещены в fixture. Результаты общего suite и smoke сохраняются
управляемой проверкой задачи.

Пользователь 18.09.2026 прямо оставил все проверки в виртуальных машинах себе и
попросил только новый релиз. Computer Use и гостевые проверки прекращены. Это не
подтверждение работы нового выпуска в Windows 11 ARM64/x64.
...
### 0.6.42 — C001: последовательный ввод

tests/tunnel-clipboard.test.mjs проверяет явную вставку ранее скопированного ID,
отказ на ключ/неверный ID/недоступный буфер без утечки содержимого, остановку перед
инструкцией API key и единственный следующий ручной вызов только с сохранённым ID.
Проверены сохранение ID при отмене и отсутствие гонки с опросом буфера.

0.6.42 / C002: targeted startup-ui — 18/18 PASS. Новые сценарии отдельно задают
platform darwin и win32: кнопка API keys вне details, инструкция Create new secret
...
0.6.43 / D001: node --test tests/tunnel-id-prompt.test.mjs tests/mac-first-run.test.mjs
...
0.6.43 / D004: tests/startup-ui.test.mjs — 20/20 PASS. Для darwin/win32 реальные
клики кнопок связаны с TunnelClipboard при пустом буфере: окно запрошено один раз,
...
0.6.44 / E003: полный Node suite — 315 tests, 313 passed, 0 failed,
2 native-Windows skipped; Electron smoke прошёл обычной командой на временном
профиле. Три предыдущие попытки достигли внешнего timeout, когда пользователь
сворачивал тестовое окно; отдельная диагностика подтвердила minimized=true.
После того как окно оставили развёрнутым, обычный неизменённый smoke прошёл.
Временный display guard был подготовлен, но не использовался. Evidence:
.harness/runtime/033-checks-044-author.json. Реальный Windows этим не проверен.

0.6.44 / F001: startup UI — 22/22. На обеих платформах новый шаг подключения
предшествует проекту, основная кнопка доступна без раскрытия справки, инструкции
различают компьютеры и не выдают локальную готовность за доступ агента к файлам.

0.6.44 / E005: парная сборка и пакетные проверки прошли; установленный Mac app
подтвердил 0.6.44 и прежний inode 398344301. F001 после проверки потребовал
исправить только список связанных документов в записи задачи. Сверка src,
resources, tests, scripts, package и lock с проверенным кандидатом дала нулевой
