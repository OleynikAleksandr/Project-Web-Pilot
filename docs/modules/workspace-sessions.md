...
Scope 029 / T004: готовность workspace может переиспользоваться в памяти при новом совпадающем отпечатке. Это состояние всей папки, а не кэш выбранного плана или recovery; sessionId/planId всё равно читаются отдельно. Ранний показ и поколение навигации интегрируются T005.

## Быстрый выбор — исходники scope 029 / T005

Подключённые проекты показывают адресованную проекцию через доверенный bundled SessionPlans и начинают loadURL до полной фоновой проверки. Последнее поколение навигации определяет store, план, URL, health и controller; чтения не удерживают очередь записей или общую очередь IPC. Ошибка фона не закрывает сохранённый чат и предлагает повтор/Доктора. Отмена Settings/выбора сохраняет загруженный DOM. Первое подключение и legacy adoption остаются строгими. Подробности и границы незавершённой поставки — session-opening-performance.md.
