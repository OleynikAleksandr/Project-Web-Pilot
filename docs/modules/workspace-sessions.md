...
Пользователь поручил отдельную кнопку «Цвета чата…» в Settings и перемещаемое немодальное окно по образцу архива. Четыре независимых nullable HEX-значения: background (общая подложка), userBackground (плашка пользователя), userText (его текст), assistantText (текст агента). Изменение применяется сразу; палитра сохраняется локально между запусками, переходами и обновлениями. Сброс каждого цвета и всей палитры возвращает штатный CSS ChatGPT. Повторный вызов поднимает единственное окно.

Facade оформления: ChatColors.set(palette)/apply()/dispose(), ChatColorsWindow.open()/close()/publish(). Входы — четыре валидированных #RRGGBB или null; выходы — user-origin CSS только для https://chatgpt.com и локальные settings. Renderer редактора имеет отдельный preload с узкими IPC; удалённая страница не получает локальный API. CSS не меняет сообщения, модель, режим Chat/Work, recovery, прокрутку или инструменты. Код, ссылки и SVG сохраняют собственные цвета. Основной поток сериализует сохранение с другими настройками. Внешний DOM может потребовать адаптации после обновления ChatGPT; native Windows и реальный аккаунт относятся к пользовательской приёмке.
