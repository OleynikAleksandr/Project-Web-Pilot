...
Согласован контракт [Первый запуск Web Pilot на чистой системе](modules/first-run-onboarding.md): владелец Release & Local Installation, смежные части Runtime Lifecycle, Workspace Setup и Workspace & Sessions. Подготовлен план этой сессии; реализация и новые испытания ещё не начаты. Проверки и исправления повторяются на свежих клонах подготовленного Base, пока приложение не проведёт пользователя от установки до первого рабочего проекта. Реестр фактов — docs/CLEAN_INSTALL.md.
