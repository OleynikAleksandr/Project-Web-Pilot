...
Согласован контракт [Первый запуск Web Pilot на чистой системе](modules/first-run-onboarding.md): владелец Release & Local Installation, смежные части Runtime Lifecycle, Workspace Setup и Workspace & Sessions. Первое испытание выявило затруднения установки и ожидания страницы; первая итерация мастера реализована и поставлена в macOS 0.6.32. Повторное ручное испытание на свежем клоне ещё впереди, Windows-итерация не начата. Проверки и исправления повторяются на свежих клонах подготовленного Base, пока приложение не проведёт пользователя от установки до первого рабочего проекта. Реестр фактов — docs/CLEAN_INSTALL.md.
