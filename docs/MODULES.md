...
Согласован контракт [Первый запуск Web Pilot на чистой системе](modules/first-run-onboarding.md): владелец Release & Local Installation, смежные части Runtime Lifecycle, Workspace Setup и Workspace & Sessions. После macOS-мастера 0.6.32 пустая панель повторилась. Диагностическая итерация 0.6.33 выпущена для обеих платформ; в macOS-мастере добавлены копирование отчёта и подсказки по наблюдаемому состоянию. Следующий T014 сначала собирает отчёт в том же госте, затем подтверждает полный путь на свежем клоне Base. Windows-мастер и полное end-to-end ещё впереди. Реестр фактов — docs/CLEAN_INSTALL.md.