...
Согласован контракт [Первый запуск Web Pilot на чистой системе](modules/first-run-onboarding.md): владелец Release & Local Installation, смежные части Runtime Lifecycle, Workspace Setup и Workspace & Sessions. После пустой панели 0.6.33 пользователь подтвердил открытие сайта в Safari гостя. Итерация 0.6.34 обеих платформ ограничивает ожидание первого документа и добавляет один автоматический повтор соединения; macOS-мастер использует готовый DOM для проверки входа. Полный гостевой результат и Windows-мастер остаются незавершёнными. Следующий T015 проверяет новую поставку, затем весь путь на свежем клоне Base. Реестр фактов — docs/CLEAN_INSTALL.md.
