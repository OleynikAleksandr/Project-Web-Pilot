...
Согласован контракт [Первый запуск Web Pilot на чистой системе](modules/first-run-onboarding.md): владелец Release & Local Installation, смежные части Runtime Lifecycle, Workspace Setup и Workspace & Sessions. В 0.6.37 исправлен показ установщика Apple, первое открытие без проекта ведёт прямо на вход. В 0.6.35 подтверждены сайт и аккаунт на исходном клоне без Chrome; причина прежних сетевых таймаутов не установлена. Видимость окна 0.6.37 и полный чистый путь ожидают T017; Windows-мастер остаётся незавершённым. Реестр — docs/CLEAN_INSTALL.md.
