...
Согласован контракт [Первый запуск Web Pilot на чистой системе](modules/first-run-onboarding.md): владелец Release & Local Installation, смежные части Runtime Lifecycle, Workspace Setup и Workspace & Sessions. 18.09.2026 подготовлена поставка **0.6.38** для macOS arm64 и Windows x64; ZIP находятся в ~/Downloads/WebPilot-0.6.38/. Исправлены отказ русского системного окна ввода туннеля и повторная кнопка установки Apple. После принятого запроса установки Web Pilot автоматически проверяет Git, затем оставляет «Проверить и продолжить». На 0.6.37 в UTM WebPilot-Test-macOS 02 подтверждены вход ChatGPT и готовность компонентов после Apple; прежний MAC_RUNTIME_EXTERNAL_MODIFIED не повторился. Вывод окна Apple вперёд не подтверждён: сработало предупреждение, пользователь завершил установку. Этот дефект фокуса остаётся отдельным наблюдением. Новый чистый проход 0.6.38 до действия с файлом ещё не выполнен; Windows native и полный Windows-мастер также не проверены. Исправления этой сессии — first-run-corrections-032; реестр — docs/CLEAN_INSTALL.md.
