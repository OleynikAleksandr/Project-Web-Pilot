...
### Историческое состояние прототипа 0.6.0
...
## Фильтр строк вызовов инструментов — T032 / hidden-tool-scroll-023

Web Pilot хранит локальный флаг `hideToolCalls` в `settings.json` и публикует его только в snapshot sidebar. Главный процесс после загрузки и внутристранничной навигации ChatGPT устанавливает небольшой DOM-наблюдатель только на `https://chatgpt.com/`: он ищет кликабельные элементы `button`, `[role=button]` и `summary` с локализованными подписями вызова инструмента (`Вызываемый инструмент`, `Called tool`, `Tool call`). При скрытии marker ставится не только на control, но и на максимально высокий безопасный ancestor, чей нормализованный текст всё ещё совпадает с текстом control. Подъём останавливается до корня user/assistant message, поэтому обычное содержимое сообщения не скрывается, а служебный tool-only контейнер полностью исключается из layout и `scrollHeight`.

При отключении настройки оба marker снимаются и исходный layout немедленно восстанавливается. После hide/restore фильтр вызывает только `window.__webPilotConversationAutoScroll.refresh()`: уже существующий conversation auto-scroll пересчитывает фактический низ после изменения layout, но не включает follow принудительно, поэтому ручное чтение истории выше сохраняется. Фильтр не отменяет tool/MCP-вызовы, не меняет сообщения и не получает локальный IPC в удалённой странице. MutationObserver нужен только для новых строк, которые ChatGPT добавляет во время ответа; при повторной навигации старый observer отключается и устанавливается заново.
...
T003: новый план создаётся отдельным каноническим файлом, service commit ограничен адресованным путём; просмотр/выбор UI не меняет глобальный план.


T004: транзакция хранит неизменный путь плана; восстановление допускается только по этому адресу. Adopt исторического архива не переписывает Git.


T005: SessionPlans — тонкий адаптер установленного Workflow Kit для чтения и вызова CLI; не хранит задачи. WorkspaceSessions исключает planView/preparedPlans из JSON store.
...
T009: sidebar отображает session-scoped view, локальное раскрытие linked plan не вызывает IPC; выбор режима переиспользует next-session-choice.
...
F001 подключает bundled Node к WorkspaceSetup. На новом профиле macOS запускается самостоятельная проверка компонентов и штатный bootstrap; загрузка страницы получает 15-секундное пояснение и отдельную ошибку. IPC startup принимает только фиксированные действия и URL, сохраняет sidebar sender guard. Секреты не запрашиваются Electron. Старые проекты открываются обычным путём; инспекция начальной готовности не подменяет доставку.


## Первый запуск / T006

T006: автономный startup view подключён отдельным ES module в существующий sidebar. Новый профиль получает четыре последовательных шага: аккаунт (вход/регистрация), компоненты, личный tunnel, подключение плагина и первый проект. Старый свернутый context card на этом экране скрыт; поля ключа в renderer отсутствуют. Управление существующими проектами сохраняется, мастер доступен через «Начальная настройка».


## Первый запуск / T007

T007: MacRuntimeBootstrap.configureTunnel запускает отдельный mac-first-run.py через Python уже проверенного runtime. Worker показывает два нативных диалога, второй со скрытым вводом, затем использует существующий operation_lock/configure_tunnel. В Electron возвращается только configured/cancelled. Секрет не проходит через argv, renderer или диагностический stderr. Рабочие службы не останавливаются для скрытой перенастройки.


## Первый запуск / C001

C001: отмена первого проекта/настроек возвращает мастер с новой навигацией; переход к проекту перепроверяет локальное состояние и наблюдаемый вход. На новом packaged Mac профиле штатный Electron moveToApplicationsFolder предлагает установку в Applications, отдельно спрашивает замену при конфликте и сохраняет отказ. Existing profile и рабочий постоянный app не перемещаются.


## Первый запуск / V001

V001 уточняет ожидание установки Apple: после открытия системного диалога кнопка повторного запуска установщика скрывается, остаётся проверка результата. Startup view проверяется отдельно и как часть реального Electron sidebar.

## First-run T008 — macOS 0.6.32

Первая итерация мастера включена в постоянный macOS app и отдельный ZIP через существующий release facade. В app.asar находятся StartupReadiness и автономный startup view; вне asar — mac-first-run.py, прежний control, payload/uv и новый Node 22.17.0 arm64 с лицензией/manifest. Сохранена identity корневого app и прежний профиль. Windows-пакет в этой итерации не обновлялся; повтор на чистой macOS передан T009.

## C002 — ранняя диагностика браузера

ChromiumDiagnostics запускается при создании view до первой навигации. Используются существующие native/CDP события и bounded JSONL: начало/DOM/HTTP/ошибки/renderer, безопасные коды и адреса. navigate пишет generation, размеры view, запрос и ожидание 15 секунд. Существующий локальный startup IPC копирует ограниченный отчёт текущей диагностической сессии; общие файлы logger остаются прежними. Реальная ошибка первой загрузки и успешный повтор проверяются в изолированном Electron через protocol fixture.

## C003 — сопровождение неоткрытой страницы

Startup view различает ожидание документа, неизвестный экран и наблюдаемый вход. До появления login controls скрыты инструкции нажать Войти/Sign up и проверка входа; доступны открытие/повтор и при задержке копирование отчёта. Регистрация объясняется без повторной навигации уже открытой страницы. Копирование не блокируется pending loadURL и сообщает успех только после результата IPC.

## Диагностическая итерация / V002

V002: отчёт первой загрузки проверяется до появления документа, включая незавершённое подключение CDP. UI smoke различает пустую панель и показанный вход; регистрация доступна после наблюдения login controls.

## Диагностическая поставка / B002

0.6.33 собрана для macOS arm64 и Windows x64 из общей реализации logger. Постоянный app и ZIP совпадают, source/resources сверены в обеих платформах. Mac UI включает копирование отчёта через существующий startup IPC; Windows-мастер не включён. Пустая панель в госте остаётся предметом диагностики T014.


C004: openStartupPage — узкая функция первого документа в пустом view. DOM-ready завершает первый шаг, предел ожидания 15 с, один recovery через закрытие соединений и DNS-cache с общим пределом 5 с, затем ещё до 15 с. Смена generation отменяет старую операцию. Навигация существующих проектов сохраняет прежнее ожидание loadURL.

После первого DOM наблюдение аккаунта выполняется в mainFrame: обычный webContents.executeJavaScript откладывает исполнение до окончания загрузки страницы. Это чтение только признаков login/profile; секреты и поля ввода не извлекаются. Код PAGE_RESPONSE_TIMEOUT сохраняется в раннем отчёте.

## Поставка восстановления первого документа / B003

0.6.34 содержит общий browser-startup facade в macOS arm64 и Windows x64. Проверены соответствие source/resources, версии и ZIP обеих платформ. Постоянный macOS app обновлён с сохранением identity. После DOM-ready macOS-мастер наблюдает аккаунт через mainFrame, не ожидая всех ресурсов. Windows-мастер остаётся отдельной задачей; результат в госте ожидает T015.

C005 / 0.6.35: openStartupPage оставляет один loadURL до DOM-ready, штатного отказа Chromium либо общего предела 120 секунд. Через 15 секунд соединения и DNS не сбрасываются. Main блокирует повтор первого открытия до смены generation. Публичная ошибка сохраняет безопасный код PAGE/ERR.
...
### 0.6.39 — U002: пустая карточка плана

Проекция NONE отображает один статус «План ещё не создан». Заголовок карточки
скрыт до появления scope, затем показывает цель его собственного плана.
Проверка Electron подтверждает один видимый текст NONE и возврат заголовка.
...
### 0.6.40 / P001 — порядок формы создания

Workspace Setup отображает форму по наличию выбранного parent: сначала выбор
расположения, затем имя и продолжение. Пустой parent скрывает путь и блокирует
отправку формы. Фокус переходит к имени после окончания выбора папки.

P002 / 0.6.40: main начинает каждый новый проект с `parent: null`, включая запуск при уже выбранном проекте. Системный выбор папки не получает defaultPath до явного выбора; отмена сохраняет пустое состояние. IPC preview отклоняет отсутствие абсолютного выбранного пути до проверки или изменения файлов. Выбор существующего проекта не менялся.

P003 / уточнение 0.6.40: первый выбор остаётся пустым только до сохранения явного расположения. `projectsParent` — локальная общая настройка main, записываемая существующим атомарным saveSettings после успешного выбора и восстанавливаемая при запуске только для абсолютного пути. Каждый begin-create берёт её, а не папку выбранного проекта. Старые settings без поля начинают с null. Отмена диалога сохраняет выбор; изменение заменяет его для всех следующих проектов. Текущие проекты не перемещаются. UI показывает «Изменить расположение папки для проектов» при сохранённом пути и сразу допускает имя.
...
### 0.6.42: разделение действий мастера туннеля

TunnelClipboard добавляет явную вставку ID и отдельный configureManually. Сначала
принимается только ID; переход к инструкции ключа завершает действие. Только
следующее ручное действие передаёт ранее полученный ID runtime и открывает
защищённый ввод ключа. Строки credentials остаются в main/worker; renderer получает
лишь безопасное состояние. Опрос буфера и ручной ввод взаимно исключаются.

0.6.42 / C002: общий StartupView показывает явные подшаги ID/API key. Ссылка keys
...
0.6.44 / F001: startup view показывает отдельный блок подключения MCP в ChatGPT
перед первым проектом. Основная инструкция и кнопка Plugins находятся вне
сворачиваемой справки; имя подключения выбирается по платформе. Готовность
локального туннеля не представляется проверкой облачного коннектора или записи
файла. Регистрацию и добавление подключения в чат выполняет пользователь.

0.6.44 / E004: номер package, lock и обоих packager targets синхронизирован;
персональное поле author удалено из manifest приложения. Стабильный bundle ID
сохраняет профиль установленного приложения.

## 0.6.44 — создание проекта и подключение инструментов

Служебная Git-подпись выбирается только для install в worker и только для недостающих локальных значений. Renderer/IPC/preview не содержат авторских полей. Мастер разделяет локальные службы, регистрацию MCP в аккаунте и проверку действия с файлом; сторонний Mac-коннектор не подтверждает Windows-доступ.

### 0.6.45 — инструкция разрешений MCP

Общий src/ui/index.html содержит видимый блок разрешений внутри шага подключения
MCP. В нём перечислены четыре режима Permissions и путь Settings → Plugins →
выбранное подключение → Permissions. Allow all actions — добровольный режим
без повторных подтверждений; настройки аккаунта остаются под контролем
пользователя. Facade, IPC, runtime и сохранение секретов не изменяются. Версия
package и обеих платформ синхронизирована на 0.6.45.
