...
Действующие компактные контракты: docs/architecture/OVERVIEW.md, docs/MODULES.md и docs/modules/session-owned-plans.md. Версионные разделы ниже — история; глобальный план, scopeTransition и обязательная кнопка приёмки заменены в 0.6.28 собственными планами сессий и ручным выбором продолжения.

## Состояние

На 17.09.2026 собран Project Web Pilot **0.6.29** с Workflow Kit **1.4.1**: быстрый показ собственного плана и начало открытия сохранённого чата, фоновая проверка готовности и строгая повторная проверка перед Send. Постоянный macOS app обновлён с сохранением Finder-алиаса; ZIP macOS arm64 и Windows x64 находятся в ~/Downloads/WebPilot-0.6.29/. Существующие планы сессий, подготовленные продолжения и оформление сохранены. Проверки и фактические замеры — docs/VERIFICATION.md; native Windows и чистая установка остаются отдельными.
