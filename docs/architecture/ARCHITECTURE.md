...
На 17.09.2026 выпущен Project Web Pilot **0.6.29** с Workflow Kit **1.4.1**: быстрый показ собственного плана и начало открытия сохранённого чата, фоновая проверка готовности и строгая повторная проверка перед Send. Постоянный macOS app обновлён с сохранением Finder-алиаса; ZIP macOS arm64 и Windows x64 находятся в ~/Downloads/WebPilot-0.6.29/. Существующие планы сессий, подготовленные продолжения и оформление сохранены. Проверки и фактические замеры — docs/VERIFICATION.md; native Windows и чистая установка остаются отдельными.

Подготовлены чистые macOS и Windows ARM64 в UTM, сохранены отдельные Clean-эталоны и Test-клоны. В Test-macOS распакован пакет 0.6.27. Первый запуск и проверка самодостаточности перенесены в следующий согласуемый scope. Диагностика задержек Computer Use выполнена без изменения MCP/runtime; измерения и ограничения — docs/CLEAN_INSTALL.md. Архитектура приложения в этом документальном scope не менялась; сетевая доставка MCP и формирование снимка измеряются отдельно.
